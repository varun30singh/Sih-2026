...
export class ChatbotService {
  private pythonChatbotUrl =
    process.env.CHATBOT_API_URL || 'http://127.0.0.1:5000/api';
  async checkHealth() {
    ...
    return new Promise((resolve) => {
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            const parsed = JSON.parse(rawData);
            // Return the Python chatbot response directly.
            // This keeps the response format as:
            // {
            //   success,
            //   language,
            //   message,
            ...
            resolve(
              createErrorResponse(
                'Failed to parse chatbot response',
              ),
            );
          }
        });
      ...
      req.on('error', (err) => {
        resolve(
          createErrorResponse(
            `Chatbot service error: ${err.message}`,
          ),
        );
      });
